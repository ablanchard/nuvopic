...
DO $$
BEGIN
    -- Check if column exists and is still VECTOR(128)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'faces' AND column_name = 'embedding'
        AND udt_name = 'vector'
    ) THEN
        -- Check dimension by attempting a cast; if it fails the column is already 512
        BEGIN
            -- Truncate all face data (will be regenerated by reprocessing)
            TRUNCATE TABLE faces;
            ALTER TABLE faces ALTER COLUMN embedding TYPE VECTOR(512);
        EXCEPTION WHEN others THEN
            -- Column is already VECTOR(512) or doesn't need migration
            NULL;
        END;
    END IF;
END $$;
